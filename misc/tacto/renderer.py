...
class Renderer:
    ...
    def _generate_pointcloud_from_depth(self, depth):
        # Load config
        g = self.conf.sensor.gel
        origin = g.origin
    ...
    def _init_camera(self):
        ...
        for i in range(self.nb_cam):
            ...
            camera = pyrender.PerspectiveCamera(
                yfov=np.deg2rad(cami.yfov), znear=cami.znear, aspectRatio=cami.width/cami.height
            )
            ...
            self.projection_matrix.append(camera.get_projection_matrix(cami.width, cami.height))
    ...
    def render_from_depth(self, depth, noise=True, calibration=True, scale=1.0, pc=False):
        """
        :param depth:
        :param scale:
        :return:
        ...
        if pc:
            return self._generate_pointcloud_from_depth(depth_resize)
